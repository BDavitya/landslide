...
<html lang="en">
  <head>
    ...
    <title>Dashboard - LandSlide Dashboard</title>
  ...
  <body class="d-flex">
    <div
      class="d-flex flex-column flex-shrink-0 bg-body-tertiary px-2"
    ...
    >
      ...
      <hr />
      <ul
        class="nav nav-pills nav-flush flex-column mb-auto text-center mt-4 text-gray-700 list-unstyled w-100"
    ...
    <div class="container" style="margin-left: 150px;">
      <div class="my-5">
        ...
        <div class="d-flex flex-wrap gap-4 row m-0">
          <div
            class="d-flex align-items-center py-4 px-5 bg-white shadow rounded col"
          >
            <div
              class="d-inline-flex align-items-center justify-content-center h-4 w-4"
            ...
            <div>
              <?php
                        include ("connect.php");
                        $sql = "SELECT COUNT(*) AS totalData FROM berita";
                        $q1 = mysqli_query($koneksi, $sql);
                        $data = mysqli_fetch_array($q1); 
                        $totalData = $data['totalData']?>
              <span class="d-block fs-2 fw-bold"><?=$totalData?></span>
              <span class="d-block text-muted"> Berita</span>
            </div>
          </div>
          <div
            class="d-flex align-items-center py-4 px-5 bg-white shadow rounded col"
          >
            ...
            <div>
              <?php
                        include ("connect.php");
                        $sql = "SELECT COUNT(*) AS totalData FROM oranghilang";
                        $q1 = mysqli_query($koneksi, $sql);
                        $data = mysqli_fetch_array($q1); 
                        $totalData = $data['totalData']?>
              <span class="d-block fs-2 fw-bold"><?=$totalData?></span>
              <span class="d-block text-muted">Orang Hilang</span>
            </div>
          </div>
        </div>
        <!-- <div class="d-flex align-items-center py-4 px-5 bg-white shadow rounded mt-4">
                ...
                <div>
                    <?php 
                    include ("connect.php");
                    $sql = "SELECT date_format(date, '%W, %d %M %Y') as formatdate, time_format(time, '%H:%i') as formattime, name as classname, coach as classcoach FROM classes WHERE date >=
            CURDATE() ORDER BY date"; $q1 = mysqli_query($koneksi, $sql); $data
            = mysqli_fetch_array($q1); ?>
                    <span class="d-block fs-2 fw-bold"><?= $data['formatdate'] . ' on ' . $data['formattime'] ?></span>
                    <span class="d-block text-muted">waktu terbaru terjadinya tanah longsor di Indonesia</span>
                </div>
            </div> -->
      </div>
    </div>
